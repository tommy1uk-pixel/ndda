...
3. Apply `migrations/0001_initial.sql` to a new database. For an existing deployment, apply each later migration in order through `migrations/0011_registration_payment_history.sql`.
...
6. In Cloudflare Zero Trust, protect `/admin.html` and `/api/admin/*` with an Access policy restricted to approved committee emails.
