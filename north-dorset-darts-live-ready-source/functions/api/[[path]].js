...
async function seasonArchiveSnapshot(env,seasonName){
  ...
  const transfers=await env.DB.prepare("SELECT * FROM player_transfers ORDER BY effective_date,id").all();
...
export async function onRequest(context) {
  ...
  try {
    ...
    if(method==="GET"&&segments[1]==="registration-payments"){
      if(!canWrite(admin))return json({error:"Insufficient permission"},403);const rows=await env.DB.prepare("SELECT rp.id,rp.team_id,t.name AS team_name,rp.amount_pence,rp.payment_method,rp.note,rp.received_by,rp.received_at FROM registration_payments rp JOIN teams t ON t.id=rp.team_id ORDER BY rp.received_at DESC,rp.id DESC LIMIT 1000").all();return json({payments:rows.results});
    }
    if(method==="GET"&&segments[1]==="league-expenses"){
      if(!canWrite(admin))return json({error:"Insufficient permission"},403);const rows=await env.DB.prepare("SELECT id,expense_date,category,description,amount_pence,payment_method,reference_note,entered_by,created_at FROM league_expenses ORDER BY expense_date DESC,id DESC LIMIT 1000").all();return json({expenses:rows.results});
    }
    if(method==="GET"&&segments[1]==="league-income"){
      if(!canWrite(admin))return json({error:"Insufficient permission"},403);const rows=await env.DB.prepare("SELECT id,income_date,category,description,amount_pence,payment_method,reference_note,entered_by,created_at FROM league_income ORDER BY income_date DESC,id DESC LIMIT 1000").all();return json({income:rows.results});
    }
    if (!sameOrigin(request)) return json({ error: "Invalid origin" }, 403);
