...
async function seasonArchiveSnapshot(env,seasonName){
  ...
  const [transfers,payments,expenses,income]=await Promise.all([env.DB.prepare("SELECT * FROM player_transfers ORDER BY effective_date,id").all(),env.DB.prepare("SELECT * FROM registration_payments WHERE season_name=? ORDER BY received_at,id").bind(seasonName).all(),env.DB.prepare("SELECT * FROM league_expenses WHERE season_name=? ORDER BY expense_date,id").bind(seasonName).all(),env.DB.prepare("SELECT * FROM league_income WHERE season_name=? ORDER BY income_date,id").bind(seasonName).all()]);
  return {season:seasonName,created_at:new Date().toISOString(),teams:teams.results,venues:venues.results,players:players.results,player_transfers:transfers.results,fixtures:fixtures.results,results:results.results,cup_ties:cups.results,calendar_events:events.results,scorecards:scorecards.results,match_games:games.results,player_statistics:playerStats.results,registration_payments:payments.results,league_expenses:expenses.results,league_income:income.results};
}
...
async function exportLeague(type,env){
  ...
  if(type==='backup'){const tables=['admins','applications','venues','teams','players','player_transfers','fixtures','results','rules','audit_log','cup_ties','calendar_events','match_scorecards','match_games','match_player_stats','league_notices','registration_payments','league_expenses'],backup={created_at:new Date().toISOString(),tables:{}};for(const table of tables){try{const rows=await env.DB.prepare(`SELECT * FROM ${table}`).all();backup.tables[table]=rows.results}catch(error){backup.tables[table]=[]}}return new Response(JSON.stringify(backup,null,2),{headers:{'content-type':'application/json; charset=utf-8','content-disposition':`attachment; filename="north-dorset-darts-backup-${stamp}.json"`,'cache-control':'no-store'}})}
  return null;
}
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (method === "POST" && segments.join("/") === "applications") {
      ...
      const created=await env.DB.prepare("INSERT INTO applications (name,email,town,interest_type,message,consent_at,team_name,venue_name,venue_address) VALUES (?,?,?,?,?,?,?,?,?)").bind(name,email,town,interest,clean(payload.message,1000),new Date().toISOString(),teamName||null,venueName||null,venueAddress||null).run();
